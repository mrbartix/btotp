...
def get_dependencies(): # Downloads tcl and tk with pacman and gets times new roman from the aur
 print("\n[1/6] Checking for needed pacman packages.")
 def check_installed(pkg): 
  result = subprocess.run(["pacman", "-Q", pkg],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
  return result.returncode == 0
 if not check_installed("tcl") or not check_installed("tk"):
  print("[2/6] Some dependency is not installed. Installing...\n")
  subprocess.run(["sudo","pacman","-Syu","--needed","--noconfirm","tk","tcl"])
 else: print("[2/6] Pacman dependencies already satisfied")
 print("[3/6] Checking if ttf-times-new-roman is installed...")
 if not check_installed("ttf-times-new-roman") and not check_installed("ttf-ms-fonts"):
  print("[4/6] Downloading Times New Roman from the AUR...")
  if not check_installed("git"):
   print("[5/6] git is not installed. Installing git...")
   subprocess.run(["sudo","pacman","-Syu","--needed","--noconfirm","base-devel"])
  else: print("[5/6] git is installed. Installing Times New Roman...")
  print("[6/6] Installing Times New Roman")
  subprocess.run(["git", "clone", "https://aur.archlinux.org/ttf-times-new-roman.git"], cwd=default_download_dir)
  subprocess.run(["makepkg", "-si"], cwd="ttf-times-new-roman")
 else: print("[4/6] Skipped\n[5/6] Skipped\n[6/6] Dependencies satisfied.")
...
if __name__ == "__main__":
 if os.geteuid() == 0:
  ...
  exit(1)
 print("btotp installer for Arch Linux")
 print("Make sure that you really are on an Arch based system")
 print("By 'Arch based' I mean that your system has to use pacman")
